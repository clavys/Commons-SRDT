...
public final class Strings {
   ...
   */
  @Nonnull public static final String LR = System.lineSeparator();

  /** The predicate to test an hexadecimal string. */
  @Nonnull
  private static final Predicate<String> IS_BINARY =
      Pattern.compile("^[0-9a-f]+$", Pattern.CASE_INSENSITIVE).asPredicate();
  ...
  /**
   * Encodes a {@link String} to a {@code byte} array, where each {@code byte} is represented as a
   * two-digit unsigned hexadecimal number in lower case.
   *
   * @param value the value to encode
   * @return a {@code byte} array
   * @throws NullPointerException if the {@code value} is {@code null}
   * @throws IllegalArgumentException if the {@link String#length() length} of the {@code value} is
   *     not even.
  ...
  public static byte[] toBytesBinary(final String value) {
    ...
    for (int i = 0; i < bytes.length; i++) {
      bytes[i] =
          (byte) ((toHexDigit(value.charAt(i * 2)) << 4) + toHexDigit(value.charAt(i * 2 + 1)));
    }
